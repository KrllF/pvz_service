...
func (s *APITestSuite) TestListReturns() {
	...
	s.Run("all good", func() {
		...
		s.Assert().Equal(UserID, orders.Returns[0].UserID)
		s.Assert().Equal(OrderID, orders.Returns[0].OrderID)
		s.Assert().Equal(Weight, orders.Returns[0].Weight)
		s.Assert().Equal(Price, orders.Returns[0].Price)
		s.Assert().Equal(Status, orders.Returns[0].Status)
		s.Assert().Equal(Pack.PackType, orders.Returns[0].Packaging.PackType)
		s.Assert().Equal(Pack.ExtraPack, orders.Returns[0].Packaging.ExtraPack)
	})
}
